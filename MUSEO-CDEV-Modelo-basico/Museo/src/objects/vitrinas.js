...
export function createVitrina1(scene, interactables){
  ...
  group.position.set(-7.5, 0, 6); group.rotation.y = Math.PI; group.castShadow=group.receiveShadow=true; scene.add(group);
  return { group, baseY, vitH, jabulaniModelRef: modelRef, luzObjeto };
}

export function createVitrina2(scene, interactables){
  ...
  gltf.load('/assets/models/world_cup_trophy/scene.gltf', (gltf)=>{
    ...
    trofeoModel.position.set(0,0.35,0);
...
export function createVitrinaLibertadores(scene, interactables){
  ...
  const gltf=new GLTFLoader(); const modelRef = { current: null }; gltf.load('/assets/models/copa_libertadores_2021/scene.gltf', (gltf)=>{ const copaModel=gltf.scene; copaModel.scale.set(0.09,0.09,0.09); copaModel.position.set(0,0.1605,0); copaModel.traverse(n=>{ if(n.isMesh){ n.castShadow=n.receiveShadow=true; const old=n.material; let map=null, normal=null, metal=0.9, rough=0.2; if(old){ if(Array.isArray(old)){ const f=old.find(m=>m&&(m.map||m.normalMap)); map=f?(f.map||null):(old[0]&&old[0].map?old[0].map:null); normal=f?(f.normalMap||null):(old[0]&&old[0].normalMap?old[0].normalMap:null);} else { map=old.map||null; normal=old.normalMap||null; if(old.metalness!==undefined) metal=old.metalness; if(old.roughness!==undefined) rough=old.roughness; } } if(map) try{ map.colorSpace=THREE.SRGBColorSpace; }catch(e){} n.material = new THREE.MeshStandardMaterial({ map: map||null, normalMap: normal||null, metalness: metal, roughness: rough }); n.material.needsUpdate=true; } }); innerGroup.add(copaModel); modelRef.current = copaModel; }, undefined, (e)=>console.error('Libertadores GLTF error', e));
...
export function createVitrinaAmerica(scene, interactables){
  ...
  const l2=new THREE.PointLight(0xffffff, 2.0, 4); l2.position.set(-0.3, baseY + glassH * 0.6, -0.3); group.add(l2);
  const amb=new THREE.AmbientLight(0xffffff, 0.4); group.add(amb);
  const placa = createGoldenPlaque("COPA AMÉRICA", "Trofeo de la Copa América. Información editable por el usuario."); placa.position.set(0.37, 0.9, 0); placa.rotation.set(0, -Math.PI/2, Math.PI); group.add(placa); interactables.push(placa);
  group.position.set(-7.5, 0, -3); group.rotation.y = Math.PI; group.castShadow=group.receiveShadow=true; scene.add(group);
  return { group, baseY, glassH, copaModelRef: modelRef, luces: { spot, l1, l2 } };
}

export function createVitrinaCentralEscudo(scene, interactables, ROOM){
  const group = new THREE.Group();
  ...
  group.position.set(0,0,0); group.castShadow=group.receiveShadow=true; scene.add(group);
